...
jQuery(function(){
	...
	jQuery("#fupload").submit(function(){
		if (debug) console.debug('--- requestInfo --');
		uplrun=1;
		requestInfo();
		checkIframe();
		//jQuery("#fupload,#upl_elements").hide();
    jQuery("#accepttext").hide('slow', function() { jQuery("#fuploadstop, #upl_elements").show(); });
	});
...
function requestInfo(){
	if (debug) console.debug('--- requestInfo --');
  var aurl = "/uploadprogress/info.php?ID="+ id + "&" + new Date();
	if (debug) console.debug('url: '+aurl);
	jQuery.ajax({
		url:aurl,
		...
		error: function(jqXHR, textStatus, errorThrown){
			...
			if (debug) console.debug(textStatus);
			if (debug) console.debug(jqXHR);
		}
	});
	if(uplrun == 1){
...
function setPosition(data){
	if (debug) console.debug('--- set position --');
